...
export async function POST(req: Request) {
  try {
    ...
    const links = Array.from(new Set($('a').map((_: number, element) => {
    ...
    $('video, iframe[src*="youtube"], iframe[src*="vimeo"], iframe[src*="dailymotion"]').each((_: number, element) => {
    ...
    // Analyze styles
    $('link[rel="stylesheet"]').each((_: number, element) => {
      const href = $(element).attr('href');
